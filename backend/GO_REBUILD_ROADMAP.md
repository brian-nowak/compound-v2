...
- [x] Create `internal/db/transactions.go` with functions:
  - `CreateOrUpdateTransaction(ctx, accountID, transaction)` → error (uses UPSERT)
  - `DeleteTransactionByPlaidID(ctx, plaidTransactionID)` → error
  - `GetTransactionByUserID(ctx, userID)` → []Transaction
- [x] Create `internal/handlers/transactions.go`:
  - `POST /api/items/:itemID/sync-transactions` - trigger sync for an item
  - `GET /api/transactions/:userID` - get all transactions for user
  - Sync handler implements:
    1. Get item from DB to retrieve access token and cursor
    2. Call Plaid `TransactionsSync` API
    3. Save/update added and modified transactions to DB
    4. Update cursor in DB
    5. Return counts of added/modified/removed
- [x] Add UI sync button in `client-go/src/App.jsx`:
  - "Re-sync Transactions" button for each linked item
  - Automatic transaction sync after successful account linking
  - Green success message showing transaction counts (added/modified/removed)
  - Loading states and error handling
- [ ] Handle removed transactions (currently only processes added/modified)
...
# Sync transactions
curl -X POST http://localhost:8000/api/items/1/sync-transactions
...
**Transactions:**
- `POST /api/items/:itemID/sync-transactions` - Sync transactions for item
...
- [x] Phase 5: Transaction Sync (3-4 hours) ✅ Completed
  - ✅ Transaction sync handler (`POST /api/items/:itemID/sync-transactions`)
  - ✅ Database functions for transactions (`internal/db/transactions.go`)
  - ✅ Get user transactions endpoint (`GET /api/transactions/:userID`)
  - ✅ UI sync button with auto-sync on account linking (`client-go/src/App.jsx`)
  - ✅ Success feedback showing transaction counts
  - ⚠️ Removed transactions handling (currently only processes added/modified)
- [ ] Phase 6: Testing & Refinement (2-3 hours)
